  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const fileUpload = await axios
        .post(`/users/upload/${user._id}`, uploadData)
        .then(async (data) => {
          //   console.log("uploaded file is: ", fileUpload.secure_url);
          let update = { ...user };
          update.avatar = data.data.secure_url;
  ...
  return (
    <div className={style.Profile}>
      <section className={style.Showcase}>
        ...
        >
          <h1 style={{ marginLeft: "6%" }}>Hello {user.firstName},</h1>
          <div className={style.Avatar}>
            {user?.avatar ? (
              ...
              >
                <label htmlFor={style.FileLoader}>
                  <ImUserPlus style={{ fontSize: "2.7rem" }} />
                </label>
                <input
                  id={style.FileLoader}
                  type="file"
        ...
        <div className={style.Content}>
          ...
          <p>
            ...
            {user.totalRecycled?.toFixed(2).toString()} Kilograms of recyclable
            meterial.
          </p>
          <p>Thank you!</p>
        </div>
